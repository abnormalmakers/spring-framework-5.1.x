...
public class AnnotatedBeanDefinitionReader {
	...
	<T> void doRegisterBean(Class<T> beanClass, @Nullable Supplier<T> instanceSupplier, @Nullable String name,
		...
		/**
		 * 根据指定的 beanClass 创建一个 AnnotatedGenericBeanDefinition
		 * 这个 AnnotatedGenericBeanDefinition 可以理解为一个数据结构
		 * AnnotatedGenericBeanDefinition 包含了类的其他信息，比如一些元信息 Scope Lazy
		 */
		AnnotatedGenericBeanDefinition abd = new AnnotatedGenericBeanDefinition(beanClass);
		/**
		 * 判断这个类是否需要跳过解析，直接用register方法注册的类就是bean，不需要加注解
		 */
		...
		/**
		 * 通过 generateBeanName 生成一个 bean的名字
		 */
		String beanName = (name != null ? name : this.beanNameGenerator.generateBeanName(abd, this.registry));

		/**
		 * 处理类当中的通用注解
		 * Lazy Role DependsOn Primary
		 * 处理完成之后 processCommonDefinitionAnnotations 依旧把他添加到数据结构中
		 */
		AnnotationConfigUtils.processCommonDefinitionAnnotations(abd);

		/**
		 * 如果在向容器注册注解 bean 定义时，使用了额外的限定符注解则解析
		 * byName 和 qualifiers 变量是 Annotation 类型的数组，里边存放的不仅是 Qualifier 注解
		 * 所以spring 去循环这个数组，判断注解中是否包含了 Lazy,Primary
		 */
		if (qualifiers != null) {
		 ...
		 * 可以理解为一个 Map<abd, beanName>
		 * 不过准确的说他是一个类 ，包含了 beanDefinition ， beanName ， aliases
		...
		/**
		 * ScopedProxyMode 需要结合 web 理解 (SpringMVC)
		 */
		definitionHolder = AnnotationConfigUtils.applyScopedProxyMode(scopeMetadata, definitionHolder, this.registry);

		/**
		 * private final BeanDefinitionRegistry registry;
		 * BeanDefinitionRegistry 是AnnotationConfigApplicationContext 和 DefaultListableBeanFactory 的父类
		 * 把上述数据结构注册给 registry
		 ...
		 * registerBeanDefinition(definitionHolder, this.registry) 方法就是把 BeanDefinitionHolder 这个数据结构注册给
