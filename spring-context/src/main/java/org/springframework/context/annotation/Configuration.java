 ...
 *
 *     public config(SomeBean someBean) {
 ...
 *
 *     public config(DatabaseConfig dataConfig) {
 ...
 * }</pre>
 *
 * <p>Now both {@code config} and the imported {@code DatabaseConfig} can be bootstrapped
 * by registering only {@code config} against the Spring context:
 *
 * <pre class="code">
 * new AnnotationConfigApplicationContext(config.class);</pre>
 *
 ...
 * }</pre>
 *
 * <p>When bootstrapping such an arrangement, only {@code config} need be registered
 ...
 * {@code config} and {@code DatabaseConfig} is already implicitly clear.
 *
 ...
 * &#064;ContextConfiguration(classes = {config.class, DatabaseConfig.class})
